...
/*   Updated: 2020/01/24 15:48:31 by aait-el-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	TLIST(r->scene.objects, t_object)->vectors[1] = rotatex(TLIST(r->scene.objects, t_object)->vectors[1],
											DEG_TO_RAD(TLIST(r->scene.objects, t_object)->vectors[3].x));
	TLIST(r->scene.objects, t_object)->vectors[1] = rotatey(TLIST(r->scene.objects, t_object)->vectors[1],
											DEG_TO_RAD(TLIST(r->scene.objects, t_object)->vectors[3].y));
	TLIST(r->scene.objects, t_object)->vectors[1] = vecnorm(rotatez(TLIST(r->scene.objects, t_object)->vectors[1],
											DEG_TO_RAD(TLIST(r->scene.objects, t_object)->vectors[3].z)));
}
