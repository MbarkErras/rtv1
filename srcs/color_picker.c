...
static t_vec3   diffuse(t_raytracer *r, t_list *light)
{
    ...
    return (d_to_rgb(r->hit.object->scalars[0], color));
}
...
int     color_picker(t_raytracer *r)
{
    ...
    rgb.x += 0.1  * (((int)r->hit.object->scalars[0] >> 16) & 0xFF);
    rgb.y += 0.1 * (((int)r->hit.object->scalars[0] >> 8) & 0xFF);
    rgb.z += 0.1 * ((int)r->hit.object->scalars[0] & 0xFF);
    clamping_vector(&rgb);
